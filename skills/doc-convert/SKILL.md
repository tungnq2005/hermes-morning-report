...
- **The thing to convert is a Morning Report** ("bản tin sáng nay", "the morning report", "bản tin crypto hôm qua") and no file is attached: do NOT ask for a file. That report already exists in Morning Report's history. Load `skill_view(name="morning-report")` and follow its **Export Report** workflow, which calls this skill's `convert.py` on the stored `report.md` and remembers the result so the same report is not duplicated in the user's Drive.
...
- Google Workspace access is available when `preflight` shows `google.authorized_token: true`: private Docs/Slides/Drive links are read via the authorized API, and every non-`md` target is rendered in Google. If NOT authorized, private links fail and output falls back to local rendering — then offer to connect Google **in this chat** (load `skill_view(name="guided-setup")`, Connect Google workflow; it needs no terminal and no SSH), or to enable link sharing ("Anyone with the link") for the input. Never send a non-technical user to `authorize_google.py`; that path is for an operator with SSH access. Never claim OAuth access without verifying via preflight.
- A `minimal`-scope install cannot read private links at all. When one fails that way, say so plainly and offer both fixes: upload the file directly, or reconnect Google with private-link access via `guided-setup`.
