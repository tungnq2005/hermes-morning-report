#!/usr/bin/env python3
"""Record a Morning Report run into unified history."""
...
def source_urls_from_manifest(source_manifest: dict[str, Any] | None) -> list[str]:
    if not source_manifest:
        return []
    urls: list[str] = []
    seen: set[str] = set()
    source_lists = [source_manifest.get("fetched_sources")]
    if not isinstance(source_lists[0], list) or not source_lists[0]:
        source_lists = [source_manifest.get("sources")]
    for entries in source_lists:
        ...
        for entry in entries:
            ...
            url = str(entry.get("url") or entry.get("canonical_url") or fetch.get("final_url") or "").strip()
...
def source_collection_summary(source_manifest: dict[str, Any] | None, source_urls: list[str]) -> dict[str, Any]:
    if not source_manifest:
        return {
            "source_count": len(source_urls),
            "fresh_24h_count": None,
            "source_urls": source_urls,
            "failed_urls": [],
        }
    failed_urls = list(source_manifest.get("failed_fetch_urls", []) or [])
    return {
        "source_count": int(source_manifest.get("source_count") or len(source_urls)),
        "fresh_24h_count": source_manifest.get("fresh_24h_count"),
        "search_calls_used": source_manifest.get("search_calls_used"),
        "target_fetched": source_manifest.get("target_fetched"),
        "source_urls": source_urls,
        "failed_urls": failed_urls,
        "manifest_file": "sources-manifest.json",
    }
...
def record_report_phase(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source_manifest_path = Path(state["source_collection"]["manifest_path"])
    ...
    source_urls = source_urls_from_manifest(source_manifest) or extract_report_urls(report_file)
    ...
    state["success"] = bool(record.get("success"))
    state["phase"] = "record-report" if next_action_type == "write_audio_script" else ("complete" if record.get("success") else "record-report")
    state["can_continue"] = bool(record.get("success") and audio_enabled)
    state["next_action"] = {
...
def main() -> int:
    ...
    try:
        ...
        source_manifest = load_json_file(Path(args.source_manifest) if args.source_manifest else None)
        source_urls = args.source_url or source_urls_from_manifest(source_manifest) or extract_report_urls(report_path)
        source_collection = source_collection_summary(source_manifest, source_urls)
        ...
        if args.dry_run:
            preview = {
                ...
                "source_urls": source_urls,
                "failed_urls": args.failed_url or source_collection.get("failed_urls", []),
                "source_count": source_collection["source_count"],
                "failed_url_count": len(args.failed_url or source_collection.get("failed_urls", [])),
        ...
        source_manifest_file: str | None = None
        if source_manifest is not None:
            source_manifest_file = str(run_dir / "sources-manifest.json")
            (run_dir / "sources-manifest.json").write_text(
        ...
        audio_status = args.audio_status
        if not audio_status:
            audio_status = "pending" if prefs.get("Audio summary", "").strip().lower() == "enabled" else "disabled"
        ...
        manifest = {
            ...
            "source_collection": {
                **source_collection,
                "manifest_file": "sources-manifest.json" if source_manifest_file else source_collection.get("manifest_file"),
            },
            "report": {
                "status": args.send_status,
                "file": str(local_report),
                "sha256": hashlib.sha256(report_text.encode("utf-8")).hexdigest(),
                "char_count": len(report_text),
            },
            "audio": {
                "status": audio_status,
                "script_file": audio_script_path,
                "file": audio_file_path,
                "manifest_file": str(run_dir / "audio-manifest.json") if audio_manifest is not None else None,
                "manifest_source_file": audio_manifest_source,
            },
            "delivery": {
                "channel": prefs.get("Delivery channel", ""),
                "report_send_status": args.send_status,
                "audio_send_status": None,
            },
            "config_status": state_status,
            "extra": extra,
        }
