...
printf '{"ok":%s,"os":"%s","gateway":"%s","doctor_ok":%s,"recent_reports_26h":%s,"install_grace":%s,"sleep_setting":"%s","problems":[%s]}\n' \
  "$ok" "$OS_NAME" "$active" "$doctor_ok" "$recent" "$in_grace" "$sleep_setting" "$problems_json"
